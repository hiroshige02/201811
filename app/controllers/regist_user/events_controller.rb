class RegistUser::EventsController < ApplicationController
  ...
  def edit
    if Event.find(params[:id]).regist_user_id != current_regist_user.id
      ...
      @finish_hour = @event.finish_time.hour
      k = @event.desk
  ...
  def update
    ...
    tentative_content = params[:tentative_content]
    tentative_participant = params[:tentative_participant].to_i

    if params[:a].blank? || params[:b].blank? || params[:c].blank? || params[:d].blank? || tentative_title.blank? || tentative_content.blank? || tentative_participant.blank? || dd.blank?
      flash.now[:alert] = "入力項目を確認してください"
      @event.month = a
      @event.day = b
      @start_hour = c
      @finish_hour = d
      instance_variable_set('@desk' + dd.to_s, true)
      @event.title = tentative_title
      @event.content = tentative_content
      @event.participant = tentative_participant
      @event.desk = dd
